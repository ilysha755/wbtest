...
<v-app>
 ...
 <v-container class="d-flex justify-sm-space-between flex-wrap " fluid>
    <v-card max-width="368px" class="mb-10 mr-30 " v-for="card in items" :key="card.id">
...
<div class="d-flex justify-center px-5">
  <div >
 <v-img max-width="64px" :src="card.product.image" />
  </div>
  <div>
<v-card-title class="">{{card.product.name}}</v-card-title>
     ...
     <v-chip :color="card.statusColor" class="my-5" label>{{card.statusLocalized}}</v-chip>
